...
typedef struct pvt_state
{
	...
	char			dm_tty[DEVPATHLEN];		/*!< tty for DM commands */
...
typedef struct pvt
{
	...
	ast_mutex_t		dm_lock;			/*!< dm channel lock */
...
#define VOLUME_SYNC_DONE	3
	unsigned char		last_number[15];			/*!< DM 65 answer (1 of 2 status parts) */
	unsigned char		status6a08[6];			/*!< DM 6a08 answer (2 of 2 status parts) */
	...
	unsigned int		cdma:1;				/*!< device is CDMA modem */
