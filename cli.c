...
static char* cli_show_device_settings (struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	if (pvt)
	{
		...
		ast_cli (a->fd, "  CDMA Device             : %s\n", CONF_UNIQ(pvt, cdma) ? "Yes" : "No");
...
static char* cli_show_device_state (struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	if (pvt)
	{
		...
		ast_cli (a->fd, "  pvt->dialing            : %s\n", pvt->dialing ? "TRUE" : "FALSE" );
		ast_cli (a->fd, "  pvt->ring               : %s\n", pvt->ring ? "TRUE" : "FALSE" );
		...
		ast_cli (a->fd, "    Last incoming number  : %s\n\n", pvt->last_number);
