...
static int dc_uconfig_fill(struct ast_config * cfg, const char * cat, struct dc_uconfig * config)
{
	...
	dm_tty    = ast_variable_retrieve (cfg, cat, "dm");
	imei = ast_variable_retrieve (cfg, cat, "imei");
	imsi = ast_variable_retrieve (cfg, cat, "imsi");

	if(dm_tty){
		ast_verb (3, "[%s] DM TTY is set, maybe this is CDMA phone\n", cat);
		config->cdma = 1; //ast_true ("yes");
	}
	...
	ast_copy_string (config->dm_tty,	S_OR(dm_tty,""),     sizeof (config->dm_tty));
