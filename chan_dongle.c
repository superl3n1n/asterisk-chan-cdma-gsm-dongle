...
#include "at_read.h"
#include "dmlib.h"			/* DM command library */
#include "cli.h"
#include "app.h"
#include "manager.h"
...
static void* do_monitor_phone (void* data)
{
	...
	if(pvt->cdma){
	    //ast_verb(3,"[%s] Fuck init - I'm CDMA\n",dev);
	    dm_modem_preinit(pvt);
	    if(pvt->connected){
		if(!dm_modem_init(pvt)){
		    ast_log (LOG_ERROR, "[%s] Error initializing CDMA modem, we'll try later\n",dev);
		    goto e_cleanup;
		}
		ast_verb(3, "[%s] Starting dm_monitor\n",dev);
		if (ast_pthread_create_background (&dm_thread, NULL, dm_monitor_phone, pvt) < 0)
		    {
			goto e_cleanup;
		    }
	    }else{
		ast_verb(3,"CDMA Modem pre init problems\n");
		goto e_cleanup;
	    }
	}else{
	...
	while (1)
	{
		...
		if(pvt->cdma){
			sleep(3);
			ast_verb(6, "[%s] Performing CDMA check\n",dev);
			dm_modem_check(pvt);
		}else{
...
//sleep(10); // test
		...
		ast_copy_string (PVT_STATE(pvt, data_tty),  CONF_UNIQ(pvt, data_tty), sizeof (PVT_STATE(pvt, data_tty)));
		ast_copy_string (PVT_STATE(pvt, audio_tty), CONF_UNIQ(pvt, audio_tty), sizeof (PVT_STATE(pvt, audio_tty)));
		if(CONF_UNIQ(pvt,dm_tty)[0] != 0){
			pvt->cdma=1;
			ast_copy_string (PVT_STATE(pvt,dm_tty), CONF_UNIQ(pvt, dm_tty), sizeof (PVT_STATE(pvt, dm_tty)));
		}
		resolved = 1;
	}
	return ! resolved;
...
//	&& (pvt->monitor_thread == AST_PTHREADT_NULL || (pthread_kill(pvt->monitor_thread, 0) != 0 && errno == ESRCH)))
	{
		...
		if (pvt->data_fd >= 0)
		{
			...
			{
			if(pvt->cdma){
			pvt->dm_fd = opentty(PVT_STATE(pvt, dm_tty), &pvt->dmlock);
			if(pvt->dm_fd<0) return;
			}
