...
EXPORT_DECL int start_pbx(struct pvt* pvt, const char * number, int call_idx, call_state_t state)
{
	struct cpvt* cpvt;
	...
	if (!channel)
	{
		...
		if (pvt->cdma ? dm_hang(pvt) : at_enque_hangup (&pvt->sys_chan, call_idx))
		{
